...
class SessionEvalCase(BaseModel):
    ...
    source_directory_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    source_question: str = ""
    ...
    def validate_replay_contract(self) -> SessionEvalCase:
        ...
        if self.replayable:
            if (
                ...
                or not self.profile
                or not self.task_input
                or not self.policy_version_id
            ):
            ...
            for trace in self.tool_traces:
                if trace.agent_id != self.source_task_id or trace.agent_role == "verifier":
                    raise ValueError("replay trace does not belong to the source task")
                if trace.truncated:
...
def _load_policy_version(
    ...
    if not path.is_file():
        return None, None, ["missing policy_snapshot.json"]
    try:
        payload = _read_json(path)
        version_id = payload.get("version_id") if isinstance(payload, dict) else None
    except (OSError, ValueError, TypeError):
        return None, None, ["invalid policy_snapshot.json"]
    if not isinstance(version_id, str) or not _POLICY_VERSION_RE.fullmatch(version_id):
        return None, None, [
            "invalid policy_snapshot.json: version_id is not 12 lowercase hex characters"
        ]
    ...
    try:
        ...
        return version_id, version_path if version_path.is_file() else None, [
            "missing or invalid immutable policy version"
        ]
...
def _case_id(session_id: str, source_directory_sha256: str, gap: GapEntry) -> str:
    return f"session:{session_id}:{source_directory_sha256}:{gap.id}"
...
def _write_case(project_root: Path, case: SessionEvalCase) -> Path:
    ...
    if path.is_file():
        ...
        if existing_text == text:
            return path
        try:
            existing = SessionEvalCase.model_validate(json.loads(existing_text))
        except (ValueError, TypeError):
            raise ValueError("stored evaluation case is invalid") from None
        if existing.source_directory_sha256 != case.source_directory_sha256:
            raise ValueError("stored case identity belongs to a different source directory")
...
def import_session_cases(project_root: Path, session_dir: Path) -> list[SessionEvalCase]:
    ...
    policy_version_id, policy_version_path, policy_blockers = _load_policy_version(
        project_root,
        session_dir,
    )
    reports, report_file_blockers = _load_sub_reports(session_dir)
    source_hashes = _artifact_hashes(session_dir)
    if policy_version_path is not None:
        relative_policy_path = policy_version_path.relative_to(project_root).as_posix()
        source_hashes[relative_policy_path] = _sha256_bytes(policy_version_path.read_bytes())
    ...
    source_directory_sha256 = _sha256_text(str(session_dir))
    ...
    for gap in verification.gaps:
        ...
        gap_trace_ids = list(gap.trace_ids)
        unique_gap_trace_ids = list(dict.fromkeys(gap_trace_ids))
        if len(unique_gap_trace_ids) != len(gap_trace_ids):
            _append_once(blockers, "failure has a duplicate trace reference")

        selected_traces: list[ToolTrace] = []
        for trace_id in unique_gap_trace_ids:
            ...
            if trace.agent_role == "verifier":
                continue
            if task is not None and trace.agent_id == task.id:
                selected_traces.append(trace)
                continue
            _append_once(blockers, "failure references a foreign research-task trace")
        if not gap_trace_ids:
            _append_once(blockers, "failure has no recorded tool traces")

        if task is not None:
            task_owned_trace_ids = {
                trace.id
                for trace in traces.values()
                if trace.agent_id == task.id and trace.agent_role != "verifier"
            }
            if not task_owned_trace_ids.issubset(set(unique_gap_trace_ids)):
                _append_once(blockers, "task-owned trace is not referenced by the failure")
            recorded_task_calls = len(task_owned_trace_ids)
        ...
        if not selected_traces:
            _append_once(blockers, "failure has no task-owned tool traces")
        ...
        case = SessionEvalCase(
            case_id=_case_id(session_id, source_directory_sha256, gap),
            source_session_id=session_id,
            source_directory_sha256=source_directory_sha256,
    ...
    return sorted(imported, key=lambda case: case.case_id)
...
def load_session_eval_cases(project_root: Path) -> list[SessionEvalCase]:
    ...
    if len(case_ids) != len(set(case_ids)):
        raise ValueError("duplicate stored evaluation case ids")
